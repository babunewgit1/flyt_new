document.addEventListener("DOMContentLoaded", function () {
  ...
  const oneWayForm = document.querySelector(".search_form[data-search='one_way']");
  const roundTripForm = document.querySelector(".search_form[data-search='round_way']");
  ...
  if (roundTripForm) roundTripForm.classList.toggle("active_search", !isOneWay);
  ...
  if (isOneWay && oneWayForm) {
    fillAirportBlock(oneWayForm.querySelector(".departure_block"), data.full_departure_airport_name, data.fromId, data.fromShortCode, data.formIdInput);
    fillAirportBlock(oneWayForm.querySelector(".arrival_block"), data.full_arrival_airport_name, data.toId, data.toShortCode, data.toIdInput);

    const dateEl = oneWayForm.querySelector(".one_way_date");
    const timeEl = oneWayForm.querySelector(".one_way_time");
    if (dateEl) { dateEl.classList.remove("one_way_date_placeholder"); dateEl.textContent = formatDate(data.dateAsText); }
  ...
  if (!isOneWay && roundTripForm) {
    fillAirportBlock(roundTripForm.querySelector(".departure_block"), data.full_departure_airport_name, data.fromId, data.fromShortCode, data.formIdInput);
    fillAirportBlock(roundTripForm.querySelector(".arrival_block"), data.full_arrival_airport_name, data.toId, data.toShortCode, data.toIdInput);

    const depDateEl = roundTripForm.querySelector(".round_trip_departure_date");
    const depTimeEl = roundTripForm.querySelector(".round_trip_departure_time");
    ...
    if (multipul) {
      ...
      if (data.returnDateAsText) multipul.dataset.retDate = data.returnDateAsText;
    }
